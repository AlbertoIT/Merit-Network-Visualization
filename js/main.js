...
function setupGUI(config) {
	...
	if (config.legend.nodeColor) {
		$(".coloursNode").next().html(config.legend.nodeColor);
	} else {
		//hide more information link
		$(".coloursNode").hide();
	}
...
function Cluster(a) {
    ...
    this.content = function (a) {
	    ...
	    //sigInst.settings('labelThreshold', 0); // Show all label by default when showing cluster
	...
	//sigInst.settings('labelThreshold', 8); // Reset initial setting
        this.display = !1;
        this.list.hide();
